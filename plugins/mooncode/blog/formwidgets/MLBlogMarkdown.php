<?php namespace Mooncode\Blog\FormWidgets;

use Mooncode\Blog\Models\Post;
use Mooncode\Translate\Models\Locale;

/**
 * A multi-lingual version of the blog markdown editor.
 * This class should never be invoked without the Mooncode.Translate plugin.
 *
 * @package mooncode\blog
 * @author Alexey Bobkov, Samuel Georges
 */
class MLBlogMarkdown extends BlogMarkdown
{
    use \Mooncode\Translate\Traits\MLControl;
    ...
    protected function actAsControl($switch = true)
    {
        if ($switch) {
            $this->originalAssetPath = $this->assetPath;
            $this->originalViewPath = $this->viewPath;
            $this->assetPath = '/plugins/mooncode/translate/formwidgets/mlmarkdowneditor/assets';
            $this->viewPath = base_path('/plugins/mooncode/translate/formwidgets/mlmarkdowneditor/partials');
        }
        else {
            $this->assetPath = $this->originalAssetPath;
            $this->viewPath = $this->originalViewPath;
